...
1. from 空間（いま使っている側）から、生きているオブジェクトだけを to 空間（空けてある側）へコピーする。
2. コピーに伴って、生きているオブジェクトを指していたポインタを、すべて新しい番地に書き換える。
3. from 空間にはごみしか残らないので、まるごと捨てる。from と to の役割を入れ替える（フリップ）。

ごみは「コピーしない」ことで自動的に消えます。ごみを 1 つ 1 つ探して回収するのではなく、生きているものだけを救い出し、残りは丸ごと放棄する。この発想がコピー方式の効率の源です。回収時間が「生きているデータの量」だけに比例するのはこのためです[](#cite:fenichel1969)。
...
- あるオブジェクトを初めて to 空間へコピーしたとき、from 空間に残っている古いほうのオブジェクトのヘッダに、「私はもう to 空間のここへ引っ越した」という新しい番地を書き込む。これが転送ポインタ。
...
Obj *copy(Heap *h, Obj *obj) {
    ...
    size_t size = object_size(obj); /* 種類（と長さ情報）から大きさを求める */
...
この `forward != NULL` 判定が正しく働くには、いくつかの前提があります。すべてのオブジェクトの `forward` が割り当て時に `NULL` へ初期化されていること（`allocate` で行っています）、そして `copy` に渡されるのが **from 空間にあるヒープオブジェクトだけ**であることです。即値や `NULL` を渡さないのは呼び出し側（タグを見る側）の責任として、規約を決めておきます。また、実際の C 実装では `size` をアラインメント（境界揃え）済みにすること、`to_alloc + size` が to 空間の終端を越えないことにも注意が要ります。単純な半空間方式では「生きているデータ量 ≤ from 空間の使用量 ＝ to 空間の容量」なのであふれないはずですが、`assert` で確かめておくとバグの早期発見に役立ちます（境界揃えの具体は実践編の `align8` で見ます）。

## 浅いコピーと「中身の更新」

`copy` 関数には、まだ大事な仕事が残っています。たとえばリスト `y` をコピーしたとしましょう。`memcpy` で中身をそっくり写したので、to 空間のリストが持つ要素ポインタは、まだ from 空間の番地を指したままです。
...
## 再帰を使わずに全部たどる Cheney のアルゴリズム

さて、`forward_fields` の中でまた `copy` を呼び、その先でまた `forward_fields` を呼び……と、素直に書くと再帰になります。深いデータ構造では再帰が深くなりすぎてスタックがあふれる危険があり、しかも GC は「メモリが足りないとき」に動くので、追加のメモリを使う再帰は特に避けたいところです。

この問題を、追加のメモリをほとんど使わずに解くのが、**Cheney のアルゴリズム**です[](#cite:cheney1970)。アイデアは驚くほど巧妙です。**to 空間そのものを「やることリスト（キュー）」として使う**のです。
...
この「to 空間を作業キューとして使う」工夫のおかげで、再帰も別途のスタックも要りません。生きているオブジェクトの幅優先探索（breadth-first）が、ポインタ 2 本だけで実現できるのです。なお、コピーされる順序（幅優先）はプログラムの意味にはいっさい影響しませんが、コピー後のオブジェクトの並び順、つまりキャッシュの効きやすさ（局所性）には影響しえます。「どの順に並べると速いか」は、実用 GC では性能上の論点の一つです。
...
2. すべてのルートについて、それが指すオブジェクトを `copy` し、ルートの欄を新しい番地に書き換える（このとき最初のオブジェクト群が to 空間に積まれる）。
...
- 転送ポインタで「もうコピーした」印を残し、共有や循環があっても各オブジェクトを 1 回だけコピーする。
- コピー後はオブジェクト内の各ポインタを新番地に更新する必要がある。
- Cheney のアルゴリズムは to 空間を作業キューに使い、`scan`/`alloc` の 2 ポインタだけで再帰なしに全体をたどる[](#cite:cheney1970)。
- 長所は断片化なし、生存量に比例する回収、高速割り当て。短所はメモリ半減とオブジェクト移動に伴う制約。
