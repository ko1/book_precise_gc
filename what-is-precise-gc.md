...
コピー方式はデータを動かすので、データを指していたポインタを新しい番地に書き換えなければなりません。書き換えを正しく行うには、メモリ上の「この 8 バイトはポインタだ」「この 8 バイトはただの整数だ」を正確に区別できる必要があります。
